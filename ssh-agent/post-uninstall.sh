...
if command -v systemctl >/dev/null 2>&1 && systemctl --user status >/dev/null 2>&1; then
    # systemd available - performing cleanup
    echo "systemd detected, cleaning up ssh-agent service..."
    
    # Try to stop standard service
    ...
    # Try to stop service with username
    USERNAME=$(whoami)
    ...
    echo "systemd not available, cleanup not required"
fi
